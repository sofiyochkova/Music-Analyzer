...
async def get_spotify_track_data_by_lastfm_data(lastm_data: pd.DataFrame) -> tuple[pd.DataFrame, list[tuple[str, str]]]:
    ...
    merged = lastm_data.merge(tracks_dataframe[["name", "artist", "duration", "popularity"]], on=["name", "artist"], how="left")

    return merged, uri_not_found

async def get_spotify_artist_data_by_lastfm_data(lastfm_artist_data: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    if isinstance(lastfm_artist_data, str):
        print(lastfm_artist_data)
        return None
    ...
    async with semaphore:
        tasks = [
            ...
            for _, row in lastfm_artist_data.iterrows()
        ]
    ...
    artists_dataframe = pd.concat(artists_data)
    merged = lastfm_artist_data.merge(artists_dataframe[["name", "popularity"]], on="name", how="left")

    return merged, uri_not_found

async def collect_data(tracks_data, artists_data):
    tasks = [get_spotify_track_data_by_lastfm_data(tracks_data), get_spotify_artist_data_by_lastfm_data(artists_data)]
...
def get_overall_stats_table(time_period: str, tracks_data: pd.DataFrame, artists_data: pd.DataFrame):
    ...
    tracks_data["weighted_popularity"] = tracks_data["popularity"] * (tracks_data["playcount"] / all_scrobbles)
    artists_data["weighted_popularity"] = artists_data["popularity"] * (artists_data["playcount"] / all_scrobbles)
    ...
    stats = [all_scrobbles, all_artists, average_track_popularity, average_artist_popularity]
    indexes = ["Number of scrobbles", "Number of artists", "Average track popularity", "Average artist popularity"]
